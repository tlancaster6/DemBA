...
def main():
    parser = argparse.ArgumentParser(
        description="Triplet loss-based ID correction for DeepLabCut tracklets"
    )
    parser.add_argument('tracklet_path', type=str,
                       help='Path to *el.pickle tracklet file')
    ...
    parser.add_argument('--padding', type=int, default=10,
                       help='Padding around keypoints in pixels (default: 10)')
    ...
    parser.add_argument('--min-silhouette', type=float, default=0.2,
                       help='Minimum silhouette score to assign ID (default: 0.2). '
                            'Range: -1 to 1. Recommended: 0.0 (lenient), 0.2 (moderate), 0.5 (strict)')

    args = parser.parse_args()

    # Setup paths
    tracklet_path = Path(args.tracklet_path)
    ...
    if model_path.exists() and not args.force_retrain:
        ...
        print("Loading model... (use --force-retrain to retrain)\n")
        model = SimpleCNN().to(device)
        ...
        detector = CoOccupancyDetector(tracklets, min_conf=args.conf_threshold)
    ...
    if embeddings_path.exists() and not args.force_retrain:
        print(f"\nFound existing embeddings at {embeddings_path}")
        print("Loading embeddings... (use --force-retrain to recompute)\n")
        with open(embeddings_path, 'rb') as f:
            embeddings = pickle.load(f)
        print(f"Loaded {len(embeddings)} embeddings\n")
    else:
        print("\nExtracting embeddings for all detections...")
        embeddings = extract_all_embeddings(model, tracklets, patch_extractor, device=args.device)
    ...
    # Interactive mapping
    print("\nMapping clusters to individuals...")
    ...
    visualize_embeddings(embeddings, kmeans, cluster_mapping, output_dir, args.min_silhouette)
    ...
    save_summary_report(output_dir, tracklets, embeddings, cluster_mapping,
                       id_stats, args.min_silhouette, video_path.stem)
